...
changeCostScene.enter((ctx) => {
    ctx.editMessageText('Что изменить?', Markup.inlineKeyboard([
        [
...
changeCostScene.action(actions.CHANGE_AMOUNT, (ctx) => ctx.scene.enter(scenes.CHANGE_AMOUNT));

changeCostScene.action(actions.CHANGE_SUBJECT, (ctx) => ctx.scene.enter(scenes.CHANGE_SUBJECT));

changeCostScene.action(actions.CHANGE_CATEGORY, (ctx) => ctx.scene.enter(scenes.CHANGE_CATEGORY));

changeCostScene.action(actions.CHANGE_MONTH, (ctx) => ctx.scene.enter(scenes.CHANGE_MONTH));
...
changeAmountScene.action(actions.CANCEL, (ctx) => ctx.scene.enter(scenes.CANCEL));
...
changeSubjectScene.action(actions.CANCEL, (ctx) => ctx.scene.enter(scenes.CANCEL));
...
changeCategoryScene.enter(async (ctx) => {
    ...
    ctx.editMessageText('Выберите категорию расхода', Markup.inlineKeyboard(categories));
});

changeCategoryScene.action(actions.ADD_CATEGORY, (ctx) => ctx.scene.enter(scenes.ADD_CATEGORY));
...
changeCategoryScene.action(actions.CANCEL, (ctx) => ctx.scene.enter(scenes.CANCEL));
...
changeMonthScene.action(actions.CANCEL, (ctx) => ctx.scene.enter(scenes.CANCEL));

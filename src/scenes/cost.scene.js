...
export const addCost = new Scenes.BaseScene(scenes.ADD_COST);
...
addCost.action(new RegExp(`^${actions.CATEGORY}`), (ctx) => {
...
addCost.action(MONTHS_PATTERN, (ctx) => {
  Cost.month = ctx.callbackQuery.data;
  return ctx.scene.enter(scenes.CONFIRM);
});

addCost.action(actions.ADD_CATEGORY, (ctx) =>
  ctx.scene.enter(scenes.ADD_CATEGORY)
);

addCost.action(actions.CANCEL, (ctx) => ctx.scene.enter(scenes.CANCEL));

addCost.use((ctx) => {
  ctx.reply(
    'Выберите действие, или нажмите кнопку Отмена',
    Markup.inlineKeyboard(buttons.cancel())
  );
});
